...
func genAddOccurrenceSql(classId string, cOs *[]model.ClassOccurrence) (sql string) {
	sql = "INSERT INTO `mk_class_occurrence`(`class_id`, `occurrence_time`, `teacher_id`, `fore_teacher_id`, `book_code`, `schedule_time`, `occurrence_status`, `create_at`, `updated_at`) VALUES "
	// 循环data数组,组合sql语句
	insertValuesFmt := "('%s','%s', '%s', '%s', '%s', '%s', %d, NOW(), NOW()),"
	insertValuesFmt = fmt.Sprintf(insertValuesFmt, classId, "%s", (*cOs)[0].TeacherId, (*cOs)[0].ForeTeacherId, "%s", "", consts.ClassOccurStatusNotStart)
